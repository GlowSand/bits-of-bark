...
public class BarkItem extends Item {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient && Woodbark.config.woodTypes.containsKey(Registry.BLOCK.getId(block).toString().toLowerCase())) {
            Block secondBlock = Registry.BLOCK.get(new Identifier(Woodbark.config.woodTypes.get(Registry.BLOCK.getId(block).toString().toLowerCase())));
            BlockState stateofDeez;
            
            if (secondBlock instanceof PillarBlock) stateofDeez = secondBlock.getDefaultState().with(PillarBlock.AXIS, blockState.get(PillarBlock.AXIS));
            else stateofDeez = secondBlock.getDefaultState();
            
            world.setBlockState(blockPos, stateofDeez);
            
            if (playerEntity != null && !playerEntity.getAbilities().creativeMode) context.getStack().decrement(1);
